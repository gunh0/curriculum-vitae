...
\cventry
    ...
    {Hyundai AutoEver} % Organization
    {Seoul, S.Korea} % Location
    {Dec. 2024 - Present} % Date(s)
    {
      \begin{cvitems}
        \item {AWS Native Security Solutions}
        \begin{itemize}
          \item {Operation and advancement of AWS native security solutions including GuardDuty for continuous anomaly detection (credential theft, malware, suspicious API calls, abnormal traffic, cryptomining)}
          \item {Implementation of integrated SIEM and monitoring systems via AWS Security Lake and Security Hub}
          \item {Integrated diverse log sources (CloudTrail, VPC Flow Logs, GuardDuty, Security Hub) for real-time anomaly detection and incident correlation}
          \item {Developed SOAR (Security Orchestration, Automation and Response) workflows using n8n for automated incident response and security operations efficiency}
          \item {Development of automated forensic isolation workflows with dedicated accounts for rapid incident containment}
          \item {Enhancement of web application security through AWS WAF customization}
        \end{itemize}
        \item {Cloud Center of Excellence (CCoE)}
        \begin{itemize}
          \item {Established and advanced CCoE with multi-account \& multi-region security operations}
          \item {Supported AWS MSSP (Managed Security Service Provider) capabilities, built compliance automation framework, and established enterprise-wide vulnerability management system}
          \item {Implemented IaC-based security (Terraform + AWS services)}
          \item {Built ChatOps notification system using AWS Lambda + Teams for real-time security event alerts}
        \end{itemize}
        \item {Data Security \& Privacy (DSPM/DLP)}
        \begin{itemize}
          \item {Applied data protection measures for Generative AI projects leveraging AWS Bedrock}
          \item {Implemented security controls for sensitive information, personal data, and internal assets}
        \end{itemize}
        \item {Container \& Kubernetes Security}
        \begin{itemize}
          \item {Strengthened container \& Kubernetes (EKS) security by integrating vulnerability scanning, runtime anomaly monitoring, and CI/CD pipeline security controls}
          \item {Established comprehensive security framework for cloud-native environments}
        \end{itemize}
      \end{cvitems}
    }
...
\cventry
    ...
    {Seoul, S.Korea} % Location
    {Aug. 2023 - Dec. 2024 (1y 5m)} % Date(s)
    {
      \begin{cvitems}
...
\cventry
    ...
    {Jan. 2022 - Jun. 2023 (1y 6m)} % Date(s)
    {
